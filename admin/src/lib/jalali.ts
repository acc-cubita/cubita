...
export function isoToJalali(iso: string): { jy: number; jm: number; jd: number } {
  // فقط بخشِ تاریخ. ورودی ممکن است `YYYY-MM-DD` باشد یا یک datetimeِ کامل با
  // ساعت و افست (`2027-07-22T03:29:23.079-12:00`) — و آن‌وقت `split('-')` چهار
  // تکه می‌داد و روزْ `NaN` می‌شد. تا امروز نگزیده بود چون اپِ مشتری فقط ستونِ
  // `date` را می‌فرستاد؛ اپِ ستاد `expires_at`/`created_at` را هم می‌فرستد.
  const [gy, gm, gd] = iso.slice(0, 10).split('-').map(Number)
  if (!Number.isFinite(gy) || !Number.isFinite(gm) || !Number.isFinite(gd)) {
    throw new Error(`تاریخِ نامعتبر: ${iso}`)
  }
