...
export function ShortcutsPage({
  ...
  return (
    <div className="page panels">
      ...
      <BuiltInShortcutsCard />
    </div>
  )
}

/**
 * میان‌برهای **ثابتِ** برنامه — قابلِ تعریف نیستند، پس در جدولِ بالا نمی‌آیند.
 *
 * چرا این‌جا و نه فقط در خودِ فرم: میان‌بری که فقط داخلِ کد و یک `title` زندگی
 * کند، کسی پیدایش نمی‌کند. کاربری که دنبالِ «کلیدهای میان‌بر» می‌گردد به همین
 * صفحه می‌آید و باید همه‌شان را یک‌جا ببیند.
 */
function BuiltInShortcutsCard() {
  const rows: Array<[string, string, string]> = [
    ['Ctrl + K', 'جست‌وجوی سریع (کامندپالت)', 'همه‌جا'],
    ['Enter', 'خانه‌ی بعد — از حساب مستقیم به مبلغ (شرح و مرکزِ هزینه با Tab)؛ در ستونِ مبلغِ خالی، پذیرشِ باقی‌ماندهٔ توازن و رفتن به خانه‌ی بعد', 'گریدِ سند — حالت حسابدار'],
    ['کد یا نام', 'در خانه‌ی حساب همان‌جا تایپ کنید تا فهرست فیلتر شود؛ ↑/↓ انتخاب، Enter تأیید و رفتن به مبلغ، Esc برگرداندنِ حسابِ قبلی', 'گریدِ سند — حالت حسابدار'],
    ['Shift + Enter', 'مقدارِ قبلی', 'گریدِ سند — حالت حسابدار'],
    ['Tab / Shift + Tab', 'فیلدِ بعد / قبل', 'همه‌ی فرم‌ها'],
    ['↑ / ↓', 'ردیفِ بالا / پایین در همان ستون', 'گریدِ سند — حالت حسابدار'],
    ['Tab / Shift + Tab', 'خانه‌ی بعد / قبل در گرید؛ در انتهای ردیفِ آخر، ردیفِ تازه (اگر ردیفِ آخر خالی است، به «ثبت سند»)', 'گریدِ سند — حالت حسابدار'],
    ['← / →', 'خانه‌ی کناری (← بعدی در راست‌به‌چپ)؛ داخلِ متن، اول مکان‌نما تا لبه', 'گریدِ سند — حالت حسابدار'],
    ['F2', 'ویرایشِ متنِ خانه (مکان‌نما به انتها؛ پیکان‌ها داخلِ متن می‌مانند)', 'گریدِ سند — حالت حسابدار'],
    ['F4', 'فهرستِ حساب‌های همین ردیف (از سربرگ: اولین ردیفِ بی‌حساب)', 'سندِ حسابداری — حالت حسابدار'],
    ['Alt + ↓', 'بازکردنِ فهرستِ انتخابِ همین خانه (حساب، تفصیلی، مرکز)', 'گریدِ سند — حالت حسابدار'],
    ['Ctrl + Enter', 'افزودنِ ردیف', 'گریدِ سند — حالت حسابدار'],
    ['Ctrl + D', 'تکرارِ ردیف (با مبلغ)', 'گریدِ سند — حالت حسابدار'],
    ['Ctrl + Shift + C', 'کپیِ حساب و تفصیلی از ردیفِ قبل (بدونِ مبلغ)', 'گریدِ سند — حالت حسابدار'],
    ['Ctrl + Delete', 'حذفِ ردیف', 'گریدِ سند — حالت حسابدار'],
    ['Ctrl + S', 'ثبتِ سند (از هر جای فرم، سربرگ هم)', 'سندِ حسابداری — حالت حسابدار'],
    ['Ctrl + /', 'پنجره‌ی میان‌برهای سند (همان دکمه‌ی «میان‌برها»ی سرِ کارت)', 'سندِ حسابداری — حالت حسابدار'],
    ['Ctrl + G', 'رفتن به ردیف: کادرش باز می‌شود، شماره و Enter؛ Esc به خانه‌ی قبلی برمی‌گرداند', 'گریدِ سند — حالت حسابدار'],
    ['↑ / ↓ + Enter', 'در شرحِ ردیف: انتخاب از شرح‌های تکراریِ همین سند و جلسه', 'گریدِ سند — حالت حسابدار'],
    ['Alt + ↑ / ↓', 'جابه‌جاییِ منوی فوکوس‌دار (یا فلش‌های کنارِ هر منو با ماوس)؛ ترتیب روی همین دستگاه می‌ماند', 'کارت‌های «عملیات» و «فهرست»'],
    ['Ctrl + F', 'جست‌وجو در ردیف‌های سند (حساب، شرح یا مبلغ)؛ Enter بعدی، Shift+Enter قبلی، Esc پاک‌کردن', 'سندِ حسابداری — حالت حسابدار'],
    ['کلیک روی شماره‌ی ردیف', 'انتخابِ ردیف؛ Ctrl+کلیک افزودن، Shift+کلیک بازه — جمعِ انتخاب‌شده‌ها زیرِ جدول؛ Ctrl+Delete حذفِ آن‌ها، Esc لغو', 'گریدِ سند و فهرستِ اسناد'],
    ['Esc', 'بستنِ فهرستِ انتخاب یا کشو', 'همه‌جا'],
  ]

  return (
    <SectionCard
      icon={Keyboard}
      title="میان‌برهای ثابت"
      description="این‌ها همیشه فعال‌اند و قابلِ تغییر نیستند. میان‌برهای گریدِ سند فقط در «حالت حسابدار» کار می‌کنند — انتخابش در پروفایل من."
    >
      <div className="table-scroll">
        <table className="cards-on-mobile">
          <thead>
          ...
          <tbody>
            {rows.map(([key, what, where]) => (
              <tr key={key}>
                <td className="card-title" data-label="کلید">
                  <kbd>{key}</kbd>
                </td>
                <td className="card-wide" data-label="کار">
                  {what}
                </td>
                <td data-label="کجا">{where}</td>
              </tr>
            ))}
          </tbody>
        </table>
