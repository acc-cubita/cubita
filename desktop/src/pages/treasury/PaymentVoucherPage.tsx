      ...
      <form className="invoice-form form-full" onSubmit={submit}>
        <SectionCard icon={Banknote} title="سربرگ اعلامیه" description="نوع، طرف حساب، ارز و حساب‌های تنظیم‌شده">
          <div className="invoice-form form-full">
            <label>نوع پرداخت<select value={paymentType} onChange={(e) => { setPaymentType(e.target.value as PaymentType); setContactId('') }}>
            ...
            <label>طرف حساب<select value={contactId} onChange={(e) => setContactId(e.target.value)} required><option value="">— انتخاب —</option>{contacts.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
            <label>تاریخ<JalaliDatePicker value={date} onChange={setDate} /></label>
            <label>ارز<select value={currency} onChange={(e) => void chooseCurrency(e.target.value)}><option value="IRR">ریال (IRR)</option>{(data.data?.currencies ?? []).map((item) => <option key={item.id} value={item.code}>{item.name} ({item.code})</option>)}</select></label>
            <label>نرخ به ارز پایه<NumberInput value={rate} onChange={setRate} /></label>
            <label>حساب معین<select value={counterpartyAccountId} onChange={(e) => setCounterpartyAccountId(e.target.value)}><option value="">خودکار از نوع پرداخت</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</select></label>
            <label>حساب کارمزد بانکی<select value={feeAccountId} onChange={(e) => setFeeAccountId(e.target.value)}><option value="">خودکار از تنظیمات حسابداری</option>{accounts.filter((a) => a.type === 'expense').map((a) => <option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</select></label>
            <label>تخفیف<NumberInput value={discount} onChange={setDiscount} /></label>
            {Number(discount || 0) > 0 && <label>حساب تخفیف<select value={discountAccountId} onChange={(e) => setDiscountAccountId(e.target.value)} required><option value="">— انتخاب —</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</select></label>}
        ...
        <SectionCard icon={Wallet} title="اقلام پرداخت" description="هر ابزار را جدا اضافه کنید؛ جمع از همین اقلام مشتق می‌شود.">
          <div className="invoice-form form-full">
            <label>ابزار<select value={instrument} onChange={(e) => setInstrument(e.target.value as Instrument)}><option value="cash">نقد</option><option value="bank">برداشت بانکی</option><option value="payable_cheque">چک پرداختنی جدید</option><option value="endorsed_cheque">خرج کردن چک دریافتنی</option></select></label>
            {instrument !== 'endorsed_cheque' && <label>مبلغ<NumberInput value={lineAmount} onChange={setLineAmount} /></label>}
            {instrument === 'cash' && <label>صندوق<select value={cashboxId} onChange={(e) => setCashboxId(e.target.value)}><option value="">صندوق پیش‌فرض</option>{currencyBoxes.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></label>}
            {instrument === 'bank' && <><label>حساب بانکی<select value={bankAccountId} onChange={(e) => setBankAccountId(e.target.value)}><option value="">— انتخاب —</option>{currencyBanks.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></label><label>شماره برداشت<input value={withdrawalNo} onChange={(e) => setWithdrawalNo(e.target.value)} /></label><label>کارمزد<NumberInput value={bankFee} onChange={setBankFee} /></label></>}
            {instrument === 'payable_cheque' && <><label>حساب بانکی<select value={bankAccountId} onChange={(e) => { setBankAccountId(e.target.value); setCheckbookId('') }}><option value="">— انتخاب —</option>{currencyBanks.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></label><label>دسته‌چک<select value={checkbookId} onChange={(e) => void chooseBook(e.target.value)}><option value="">بدون دسته (طبق سیاست)</option>{books.filter((b) => !bankAccountId || b.bank_account_id === bankAccountId).map((b) => <option key={b.id} value={b.id}>{b.bank_account_name} — {b.serial || `${b.first_number} تا ${b.last_number}`}</option>)}</select></label><label>شماره چک<input value={checkNo} dir="ltr" onChange={(e) => setCheckNo(e.target.value)} /></label><label>سررسید<JalaliDatePicker value={dueDate} onChange={setDueDate} /></label><label>کد صیادی<input value={sayadId} maxLength={16} dir="ltr" onChange={(e) => setSayadId(e.target.value.replace(/\D/g, ''))} /></label></>}
            {instrument === 'endorsed_cheque' && <label className="form-wide">چک موجود<select value={endorsedId} onChange={(e) => setEndorsedId(e.target.value)}><option value="">— انتخاب چک واجد شرایط —</option>{eligibleCheques.map((c) => <option key={c.id} value={c.id}>شماره {c.number} — {fa(c.amount)} — سررسید {formatJalali(c.due_date)}</option>)}</select></label>}
