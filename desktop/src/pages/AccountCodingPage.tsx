...
import { AlertTriangle, CheckCircle2, Hash, ListTree, RefreshCw, Save } from 'lucide-react'
...
/**
 * کدینگ حسابداری — ساختارِ کدِ حساب‌ها و روش‌های شماره‌گذاریِ اسناد.
 *
 * این دو کنارِ هم‌اند چون هر دو «قاعده‌ی شماره» را تعریف می‌کنند: یکی کدِ حساب در
 * چارت، دیگری شماره‌ی سریِ سندها. هر دو تصمیم‌هایی هستند که یک بار در راه‌اندازی
 * گرفته می‌شوند و بعد کمتر دست می‌خورند — یعنی جایشان تنظیمات است، نه کارِ روزانه.
 */

const fa = (n: number) => n.toLocaleString('fa-IR')

export function AccountCodingPage({ token }: { token: string }) {
  return (
    <div className="page panels">
      <PageHeader
        icon={Hash}
        title="کدینگ حسابداری"
        description="ساختارِ کدِ حساب‌ها و شماره‌ی سریِ اسناد — قاعده‌هایی که یک بار تعریف می‌شوند و بعد همه‌ی سندها از آن‌ها پیروی می‌کنند."
      />
      <Tabs
        syncPage="coding"
        tabs={[
          {
            ...
            label: 'کدینگ حساب‌ها',
            icon: ListTree,
            content: <ChartOfAccountsPanel token={token} />,
          },
          {
            key: 'numbering',
            label: 'روش‌های شماره‌گذاری',
            icon: Hash,
            content: <NumberingPanel token={token} />,
          },
        ]}
      />
    </div>
 ...
 */
function NumberingPanel({ token }: { token: string }) {
  ...
  return (
    <SectionCard
      ...
      description="شماره‌ی سندِ بعدیِ هر نوع سند. شماره فقط می‌تواند جلو برود — شماره‌ی مصرف‌شده روی سندِ ثبت‌شده نشسته است."
      actions={
                    ...
                    <td data-label="تنظیمِ شماره‌ی بعدی">
                      <div className="nm-set">
                        <input
                          ...
                          onChange={(e) =>
                            setDrafts((d) => ({ ...d, [r.doc_type]: e.target.value }))
                          }
                        />
                        <button
                          type="button"
                      ...
                      {invalid && (
                        <span className="nm-invalid">
                          باید بزرگ‌تر از {fa(r.last_number)} باشد
                        </span>
                      )}
                    </td>
                  </tr>
