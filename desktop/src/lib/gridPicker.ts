 ...
 *
 * `SearchSelect` دو شکل دارد: برای فهرستِ کوتاه `<select>`ِ بومی، برای بلند دکمه‌ی
 * پاپ‌آور (`.item-picker-trigger`). هر دو این‌جا یکی می‌شوند تا گرید و سربرگ لازم
 * نباشد بدانند کدام رندر شده. `true` یعنی انتخاب‌گری پیدا و باز شد.
 */
