...
3. **مدیریتِ پلتفرم فقط در `admin/` زندگی می‌کند.** هرگز صفحه یا رَپرِ
   `/api/admin/*` به `desktop/` اضافه نکن. `acc.cubita.ir` فقط دفترِ مشتری است و
   `admin.cubita.ir` فقط مدیریت. اندپوینت‌های ستادی توکنِ **ستادی** می‌خواهند که
   اپِ مشتری اصلاً نمی‌گیرد، پس چنین صفحه‌ای فقط ۴۰۱ می‌دهد. تستِ
   `navModel.test.ts` برگشتنشان را می‌گیرد.
4. **parityِ سه‌پلتفرمی:** هر قابلیت باید هم روی دسکتاپ و هم وب کار کند.
5. **نمای موبایل بخشی از «تمام‌شده» است:** بعد از هر تغییرِ UI، نمای `≤۷۶۰px` را
   هم بساز/راستی‌آزمایی کن.
6. **همه‌ی متنِ کاربر فارسی است.** ارقام با `toLocaleString('fa-IR')`، تاریخ‌ها
   جلالی از `desktop/src/lib/jalali`.
7. **دو نمای یک داده نساز.** اگر جدول/صفحه‌ای برای این داده هست، همان را گسترش بده.
8. **رازها هرگز کامیت نمی‌شوند.** `.env`ها نادیده گرفته شده‌اند؛ الگو در `*.env.example`.
...
بازسازیِ صفحه‌ها با تمِ اکسلیِ «سند حسابداری» (گرید، نوارِ هم‌خط، برگه‌ی ویرایشِ درجا) روشِ خودش را در
`.claude/skills/cubita-excel-theme/SKILL.md` دارد — صفحه‌ی بعدی را از آن‌جا ادامه بده.
