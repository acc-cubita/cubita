...
// ثبتِ دریافت/پرداختِ خزانه — «ثبتِ داده در حرکت»، فرمِ تک‌صفحه‌ایِ موبایل.
//
// **این همان مسیرِ دسکتاپ نیست و نباید فرض شود که هست.** دسکتاپ «رسید دریافت» و
// «اعلامیه پرداخت» را می‌زند، که `counterparty_account_id` صریح دارند: کاربر
// می‌گوید پول بابتِ چه بوده. این‌جا `POST /api/treasury/{receipts|payments}` است
// که طرفِ مقابل را **ثابت** می‌گذارد — دریافت همیشه بستانکارِ حساب‌های دریافتنی،
// پرداخت همیشه بدهکارِ حساب‌های پرداختنی.
//
// برای دامنه‌ی این اپ (وصولِ میدانی از مشتری) درست است، ولی محدودیتش عمدی است:
// قاعده‌ی ۵۲ پایگاه دانش می‌گوید جهتِ پول به‌تنهایی نباید علتِ حسابداری را تعیین
// کند. هرکس خواست این صفحه را به دریافتِ غیرِ مشتری گسترش دهد، اول باید
// علتِ اقتصادی را به بدنه اضافه کند — وگرنه سند بی‌صدا غلط می‌نشیند.
