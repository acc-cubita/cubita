...
/**
 * آیا ثبتِ تحویل واقعاً پول جابه‌جا می‌کند؟
 *
 * **این را با تستِ سرتاسری روی سرورِ واقعی پیدا کردیم، نه از روی کد.** دو مسیرِ
 * متفاوت در `services/marketplace.py` هست:
 *
 * - پخش‌کننده‌ای که `require_delivery` را روشن کرده: تأیید فقط سفارش را
 *   می‌پذیرد؛ سند، ورودِ انبار و تسویه‌ی نقد **هنگامِ تحویل** انجام می‌شود.
 * - پیش‌فرض (خاموش): *تأیید* خودش فاکتورهای دوطرفه را می‌زند و نقد را همان‌جا
 *   تسویه می‌کند. آن‌وقت `deliver_order` می‌بیند فاکتور از قبل هست و کلِ بلوکِ
 *   نقد را رد می‌کند.
 *
 * یعنی در حالتِ پیش‌فرض، مبلغی که مأمورِ حمل وارد کند **بی‌صدا دور ریخته
 * می‌شود** — نه خطایی، نه هشداری؛ فقط کاربر فکر می‌کند پولی ثبت کرده. پس فرم
 * باید بداند کدام مسیر است و در حالتِ دوم اصلاً مبلغ نپرسد.
 *
 * علامتش وجودِ فاکتورِ خرید است: پر بودنش یعنی تسویه قبلاً انجام شده.
 */
export const settlesOnDelivery = (order: {
  retailer_purchase_invoice_id: string | null
}): boolean => order.retailer_purchase_invoice_id === null
