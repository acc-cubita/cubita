...
|---|---|---|---|
| Claude Opus 5 | مهاجرت `0136` (پشتِ `0134`؛ **اگر PR #47 با `0135` زودتر merge شود، این باید پشتِ `0135` rebase شود**)، `models/payroll.py`، `schemas/payroll.py`، `routers/payroll.py`، `services/payroll.py`، `audit.py`، `PayrollRefPages.tsx`، انتهای `api.ts`؛ آزادسازی `pytest` | فصلِ «تعریف شعب بیمه و مالیات»: پیوندِ شعبه به طرف حساب، حسابرسیِ شعبه، مسیرِ ویرایش، و دیده‌شدنِ شعبه در فایلِ بیمه و مالیات | ۱۴۰۵/۰۶/۲۲ |
