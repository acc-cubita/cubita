...
هر دو توسعه‌دهنده کلیدِ SSH خودشان را روی VPS دارند و می‌توانند استقرار کنند:

```bash
# ساختِ آرشیوها از ریشه‌ی مخزن
tar -czf /tmp/cubita_code.tgz -C backend app alembic alembic.ini scripts
tar -czf /tmp/cubita_web_production.tgz -C desktop/dist .
scp -i ~/.ssh/<کلیدِ تو> /tmp/cubita_{code,web_production}.tgz root@<آدرسِ سرور>:/tmp/
ssh -i ~/.ssh/<کلیدِ تو> root@<آدرسِ سرور> '/opt/hesabdari/deploy.sh production'
```

`deploy.sh` قبل از هر کاری پشتیبان می‌گیرد و اگر هر گامی بشکند خودکار برمی‌گردد.

**دو چیز که با دسترسی هم باید رعایت شوند:**

- **قبل از استقرار اعلام کن.** دو استقرارِ همزمان یعنی یکی سرویس را وسطِ مهاجرتِ
  دیگری ری‌استارت می‌کند — و `deploy.sh` از این محافظت نمی‌کند.
- **`deploy.sh demo` را اجرا نکن** — گاردش درِ تریال را می‌شکند
  ([CLAUDE.md](CLAUDE.md) قیدِ ۱). تازه‌سازیِ demo فقط با `rsync`.

## چیزهایی که دسترسیِ جدا لازم دارند

اینها روی VPS نیستند و با کلیدِ SSH باز نمی‌شوند:

- **انتشار در کافه‌بازار** — حسابِ توسعه‌دهنده‌ی کافه‌بازار + کلیدِ امضای اپ
  (keystore، عمداً در مخزن نیست)
- **پنلِ زرین‌پال، ملی‌پیامک، و دامنه‌ها** — حساب‌های جداگانه

مقادیرِ زنده‌ی این‌ها در `backend/.env` روی سرور است. عوض‌کردنشان روی پولِ واقعی
و پیامکِ واقعی اثر می‌گذارد — قبلش بپرس.
