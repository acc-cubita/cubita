...
| مالیات بر ارزش افزوده | #221 | د: برگه‌ی اظهارنامه (بخش + برگشتِ منفی + جمعِ بخش، خالص در پانویس با نشانِ پرداختنی/استردادی)؛ «همه‌ی گزارش‌ها» هم فهرستِ اکسلی شد | [VatPage.tsx](../../../desktop/src/pages/accounting/VatPage.tsx) |
| گزارش دفتر | #222 | د: روزنامه با سرگروهِ سند و ردیف‌هایش در یک گرید؛ کل/معین/تفصیلی گریدِ گردش با مانده‌ی اول، جمع و انتخاب؛ پارامترِ هر دفتر در سطرِ اولِ سربرگ | [LedgerReportPage.tsx](../../../desktop/src/pages/accounting/LedgerReportPage.tsx) |
| درختواره حساب‌ها | #223 | ب روی درخت: ویرایشِ درجای کد/نام/عنوانِ دوم/ماهیت/وضعیت، «+» زیرِ هر سرفصل با کدِ پیشنهادی، ذخیره‌ی پشتِ‌سرِ‌هم (کد با مسیرِ جدا)، ویژگی‌ها در ستونِ خودشان | [ChartOfAccountsPage.tsx](../../../desktop/src/pages/accounting/ChartOfAccountsPage.tsx) |
| دفاتر تجارت الکترونیک | #224 | د: گریدِ دفترِ حساب (`lr-ledger`) بی مانده — شماره‌ی ردیفِ پیوسته، شماره و تاریخِ سند فقط روی ردیفِ اولِ هر سند، دامنه‌ی «همه/دائم/موقت» که دفترِ جدا می‌سازد، ساختنِ ۵۰۰تایی با چاپ و CSVِ کلِ دفتر | [LegalBooksPage.tsx](../../../desktop/src/pages/accounting/LegalBooksPage.tsx) |
| بررسی یکپارچگی | (این PR) | د: یک گرید برای نُه بررسی — سرگروهِ بازوبسته با چیپِ وضعیت و یافته‌ها زیرش، یافته‌ها اول (خطا، هشدار، سالم)، «جمعِ دفتر» با نتیجه‌ی کل در پانویس، مقصدِ یافته سند/کاردکس/دفتر | [IntegrityPage.tsx](../../../desktop/src/pages/accounting/IntegrityPage.tsx) |
...
- **شبکه‌ی خانه‌هایی که ممکن است سطرِ آخرشان پر نشود** (فهرستِ گزارش‌ها): خطِ میانِ خانه‌ها را با `outline`ِ خودِ خانه و
  `gap: 1px` بکش، نه ترفندِ «زمینه‌ی رنگِ خط + gap» — آن ترفند خانه‌ی خالیِ ته شبکه را لکه‌ی خاکستری می‌کند. و
  `repeat(auto-fit, minmax(0, 1fr))` صدها ستونِ خالی می‌سازد؛ برای «یک ستون به‌ازای هر فرزند» `grid-auto-flow: column` و
  `grid-auto-columns` بگذار.
- **دو انتخاب‌گرِ هم‌معنا زیرِ هم نگذار:** اگر فهرستِ بالای صفحه همان گزینه‌ها را دارد، انتخاب‌گرِ کامپوننت را با پارامتر خاموش کن
  (`Reports picker={false}`) و در میزبانِ دیگر نگهش دار.
- **دو عددِ پشتِ‌هم در یک خانه** (شماره‌ی سند و تاریخ در سرگروهِ روزنامه) را `unicode-bidi: isolate` کن؛ وگرنه الگوریتمِ دوجهته
  آن‌ها را یک عدد می‌بیند («۱۵۹۱۴۰۵/۰۷/۰۳») و فاصله‌ی میانشان هم کمکی نمی‌کند.
- **برگه‌ی ویرایشی روی درخت** (درختواره): ردیفِ خالیِ ته ندارد — ردیفِ تازه با «+»ِ هر ردیف یا Ctrl+Enter **ته زیرشاخه‌ی مادر**
  می‌نشیند (`sheetRows`)، و زیرِ ردیفِ تازه‌ی ذخیره‌نشده ردیف ساخته نمی‌شود (مادر شناسه ندارد). کدِ پیشنهادیِ سرور ذخیره‌نشده‌ها را
  نمی‌بیند؛ دو ردیفِ تازه زیرِ یک سرفصل یک کد می‌گرفتند (`nextFreeCode`). `useSheetNav.onAppendRow` ردیف را نمی‌داند — ردیفِ فوکوس‌دار
  را با `onFocus`ِ گرید در یک ref نگه دار.
- **تورفتگیِ درخت با آیکون:** آیکونِ پوشه‌ی سرفصل دقیقاً یک پله‌ی تورفتگی جا می‌گرفت و متنِ فرزند هم‌ترازِ مادر می‌شد. دکمه‌ی بازوبسته و
  جای‌خالیِ برگ را هم‌عرض بگذار و آیکونِ اضافه نگذار (سرفصل را پررنگ کن).
- **R3ِ ممیز و ردیفِ جدا:** سلولی که در کامپوننتِ ردیفِ جدا و **بعد از** `</table>` است «درونِ `table-plain`» شمرده نمی‌شود؛ مثلِ
  `AnalyticsPage` `card-title`/`card-actions` یا `data-label` بگذار (یا ویژگیِ template در همان خطِ `<td`).
