...
- **گریدِ فقط‌خواندنیِ پهن‌تر از قاب (گردشِ حساب):** در RTL شروعِ لغزش سمتِ راست است و ستون‌های مبلغ (چپ) اول بیرون
  می‌مانند. آن‌ها را با `position: sticky; inset-inline-end` میخ کن (عرضِ پیکسلی و در `LAYOUT.fixed`، تا جای میخ
  پایدار بماند) و زمینه‌ی خانه‌ی میخ‌شده را **مات** بساز: `--accent-bg` نیمه‌شفاف است و ستونِ زیرش پیدا می‌شود — رنگِ
  حالِ ردیف را با `linear-gradient(...), var(--surface)` رویش لایه کن. در کارتِ موبایل، زمینه و خط و میخ همه برداشته شوند.
- **جمع/مانده‌ی گزارش در خودِ گرید، نه کارتِ خلاصه:** ردیفِ بالا («مانده‌ی اول دوره») و `tfoot`ِ چسبنده («جمعِ بازه») هم‌ستونِ
  مبلغ‌ها — از ارقامِ سرور، و از لحظه‌ی انتخاب (منتظرِ دفتر نمی‌ماند). `tfoot` در کارتِ موبایل `display: block` می‌خواهد.
- **قابِ کناریِ باریک:** پنهان‌کردنِ ستون با `@container` روی خودِ قاب، نه `@media` روی پنجره — پهنای قاب به نسبتِ
  تقسیم و منوی کناری بستگی دارد (درختِ مرور حساب‌ها).
- **سربرگِ فیلترِ گزارش:** `jh-bar jh-bar--report` با سطرهای آزاد (`--rh-cols`، کوئریِ ظرف برای شکستن)؛ بازه با
  `RangeCells` (kit) و فیلترها با `<ReportFilterBar variant="cells">`. خانه‌ی `<label>` را `gap: 0` کن — `label:has(> select)`ِ
  سراسری فاصله‌ی ۶px می‌دهد و سطر از خانه‌های `<div>` بلندتر می‌شود. `<select>`ِ بومی در `jh-bar` قاعده‌ی بی‌قابِ خودش را می‌خواهد.
- **سرستونِ دوطبقه (تراز):** `thead` را یک‌جا `position: sticky` کن، نه تک‌تکِ `th`ها — سطرِ دوم باید `top`ِ برابرِ ارتفاعِ سطرِ اول
  می‌گرفت که به padding و فونت بسته است. `th`ِ میخ‌شده درونِ `thead`ِ چسبنده هم افقی میخ می‌ماند. و قاعده‌ی `:first-child`ِ
  `xl-grid` مرزِ آغازِ **اولین `th`ِ سطرِ دوم** را برمی‌دارد، در حالی که آن خانه بعد از خانه‌های `rowSpan`ِ سطرِ اول نشسته است — مرزش را
  صریح برگردان (`.br-gs`).
- **ستون‌های حسابِ میخ‌شده سمتِ راست (RTL):** وقتی متن مهم‌تر از مبلغ است (تراز: «کد و نام» هویتِ ردیف‌اند)، ستون‌های آغاز را با
  `inset-inline-start` میخ کن — برعکسِ گردشِ حساب که مبلغ‌ها را سمتِ چپ میخ کرد. سایه‌ی لبه سمتِ پایان (`-4px 0 …`).
- **ردیفِ فعالِ کیبورد فقط با فوکوس رنگ بگیرد** (`.br-scroll:focus-within`)، وگرنه ردیفِ اولِ رنگی در بارگذاری «انتخاب» خوانده
  می‌شود. قاعده‌ی انتخاب را با همان اختصاصیت و بعد از آن بگذار، و قاعده‌ی خنثی‌سازیِ کارتِ موبایل را هم‌وزنِ هر دو کن
  (`:is(*, :focus-within)`) — وگرنه ته‌رنگِ گرید در کارت برمی‌گردد.
- **سنجشِ توازن فقط روی کلِ دفتر:** جمعِ ردیف‌های فیلترشده (نوعِ مانده، جست‌وجو) به‌تعریف ناتراز است؛ `balanceCheck(totals, complete)`
  در این حالت «فیلترشده» می‌گوید نه «ناتراز».
- **پنهان‌کردنِ ستون در `table-layout: fixed`:** `display: none` روی خانه‌ها آن‌ها را از شبکه‌ی جدول بیرون می‌برد و بقیه‌ی
  خانه‌ها **یک ستون جابه‌جا** می‌نشینند، و `col`ِ پنهان عرضش را نگه می‌دارد. ستون را جمع کن: `col { width: 0 }` و خانه‌ها
  `padding: 0; border: 0; font-size: 0` (ستونِ کدِ صورت‌های مالی در موبایل).
- **خانه‌ی `rh-seg` که در سطرش تنهاست** کش نمی‌آید؛ ارتفاعش را `min-block-size` بده نه `block-size` (که `height: auto`ِ بعدی
  بی‌صدا لغوش می‌کند). پیش‌تنظیم‌های بازه در قابِ ≤۹۰۰ همین‌طور ۲۰پیکسلی شده بودند.
- **صورتِ مالی کارت نمی‌شود:** سه ستون در موبایل جا می‌شوند؛ `table-plain` و جمع‌کردنِ کد، بهتر از کارتی به‌ازای هر قلم.
  عرضِ برگه را محدود کن (۷۸۰px) تا مبلغ از شرح دور نیفتد — و قابِ لغزش را هم (`:has(> table.rp-statement)`)، وگرنه کنارش خانه‌ی خالی می‌ماند.
