...
class EmployeePatch(BaseModel):
    """ویرایشِ پرونده‌ی کارمند — **فقط چیزهایی که مالِ خودِ پرونده‌اند.**

    نام، نام خانوادگی و کدِ ملی عمداً این‌جا نیستند: آن‌ها واقعیتِ *شخص*‌اند و
    روی طرف‌حساب می‌نشینند، و
    [`employee_identities`](../services/payroll.py) هم برای هر خروجیِ قانونی
    از همان‌جا می‌خواند. اگر این‌جا هم قابلِ ویرایش بودند، دو نمای یک داده
    می‌ساختیم که دیر یا زود از هم عقب می‌مانند — همان چیزی که کلِ پیوندِ
    «کارمند = نقشی روی طرف‌حساب» برای جلوگیری از آن ساخته شد.

    برای اصلاحِ نام: `PATCH /api/contacts/{contact_id}`.

    **هیچ فیلدی اجباری نیست و نفرستادن یعنی «دست نزن».** فرمی که فقط شماره‌حساب
    را عوض می‌کند نباید نگرانِ برگرداندنِ `is_active` به پیش‌فرض باشد.
    """

    hire_date: date | None = None
    #: تاریخِ خروج. `service_end_date`ِ حکم هم همین را پر می‌کند؛ این‌جا برای
    #: وقتی است که خروج حکمِ تازه‌ای ندارد یا تاریخش اشتباه ثبت شده.
    termination_date: date | None = None
    bank_account_number: str | None = None
    phone: str | None = None
    email: str | None = None
    #: **تنها گاردِ صدورِ فیش.** `payroll.py` با همین ستون تصمیم می‌گیرد چه کسی
    #: فیش بگیرد، و تا امروز هیچ مسیری نمی‌نوشتش — یعنی کارمندی که رفته بود
    #: تا ابد فیش می‌گرفت.
