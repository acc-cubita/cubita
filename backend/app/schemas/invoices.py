...
class PurchaseInvoiceLineIn(BaseModel):
    ...
    #: **اشانتیون — یادداشت، نه ورودیِ محاسبه (§۲۵).** کارتنِ رایگان ردیفِ
    #: صفرقیمت نمی‌شود (میانگینِ موزون را بی‌توضیح پایین می‌کشد)؛ تعداد ۱۱ با
    #: تخفیفِ بهای ۱ واحد ثبت می‌شود و بهای واقعی خودبه‌خود درست درمی‌آید.
