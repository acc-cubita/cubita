...
DEFAULT_ROLES: list[dict] = [
    ...
    },
    {
        "key": "auditor",
        "name": "حسابرس",
        # نقشِ حسابرسِ کوبیتا: با تأییدِ قراردادِ حسابرسی، یک عضویتِ **موقت** با همین
        # نقش در کسب‌وکارِ مشتری ساخته می‌شود تا حسابرس دفتر را ببیند.
        #
        # سه تصمیم:
        #   ۱. **بدون wildcard** — به همان دلیلی که از نقشِ «دمو» برداشته شد: «*»
        #      ماژول‌هایی را هم می‌پوشاند که هنوز ساخته نشده‌اند.
        #   ۲. **بدونِ `users`** — حسابرس نباید بتواند دسترسیِ کسی (از جمله خودش)
        #      را عوض کند.
        #   ۳. **فقط `view`** جز `assurance` که `refresh` هم دارد: اجرای دوباره‌ی
        #      بررسی‌ها چیزی در دفتر نمی‌نویسد، فقط عکسِ تازه‌ای می‌گیرد.
        #
        # `payroll: view` عمدی است — حسابرسیِ واقعی بدونِ دیدنِ حقوق ممکن نیست — و
        # به همین دلیل فرمِ درخواست صریح به مشتری می‌گوید حسابرس چه چیزی خواهد دید.
        "permissions": {
            "accounting": ["view"],
            "invoices": ["view"],
            "inventory": ["view"],
