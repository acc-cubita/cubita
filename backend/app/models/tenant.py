...
class Tenant(UUIDPKMixin, TimestampMixin, Base):
    ...
    #: صنفِ ریز — «این کسب‌وکار چه می‌فروشد» (app/services/trades.py). با `industry`
    #: اشتباه نشود: آن قالبِ ماژول‌هاست و پنج گزینه دارد؛ این ~۹۰ گزینه دارد و کارش
    #: وصل‌کردنِ دو کسب‌وکار در بازارِ پخش است (پخش‌کننده اعلام می‌کند به کدام اصناف
    #: جنس می‌دهد).
    #:
    #: **`NULL` یعنی «هنوز اعلام نکرده»، نه «هیچ‌کدام».** همه‌ی حساب‌های موجود
    #: این‌اند، و بازار با آن‌ها مثلِ قبل رفتار می‌کند: چیزی پنهان نمی‌شود. اگر این
    #: ستون مقدارِ پیش‌فرض می‌گرفت، آن دو حالت یکی می‌شدند و دیگر نمی‌شد فهمید
    #: کسب‌وکاری واقعاً صنفش را انتخاب کرده یا نه.
...
class Membership(UUIDPKMixin, TimestampMixin, Base):
    ...
    #: پایانِ مهلتِ این عضویت. `NULL` یعنی بی‌انقضا — یعنی رفتارِ هر عضویتی که
    #: تا امروز ساخته شده، دست‌نخورده.
    #:
    #: **چرا ستونِ عمومی و نه چیزی مخصوصِ حسابرسی:** «دسترسیِ موقت» مفهومِ خودِ
    #: عضویت است نه مفهومِ یک ماژول. و مهم‌تر، تنها نقطه‌ای که دورزدنی نیست
    #: `get_principal` است که در هر درخواست عضویت را دوباره می‌خوانَد؛ سنجشِ
    #: انقضا آن‌جا یک مقایسه‌ی درون‌حافظه‌ای روی ردیفِ از قبل بارگذاری‌شده است و
    #: **هیچ کوئریِ اضافه‌ای** به هیچ کاربری تحمیل نمی‌کند. اگر به‌جایش هر درخواست
    #: می‌خواست «قراردادِ حسابرسی» را بخواند، کلِ سامانه هزینه‌ی یک قابلیت را
    #: می‌داد.
    expires_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )

    #: کارت‌های «شروعِ کارِ تازه»ی داشبوردِ همین کاربر در همین کسب‌وکار — فهرستِ
    #: شناسه‌ی مقصد (`page` یا `page/section`) به همان ترتیبی که نشان داده می‌شود.
    #:
    #: **چرا روی عضویت و نه روی کاربر:** کارت‌ها به ماژول‌های *این* کسب‌وکار اشاره
    #: می‌کنند. یک حسابدار که در دو شرکت عضو است، در شرکتی که انبار ندارد نباید
    #: کارتِ «رسید انبار» ببیند. جایِ درستِ چنین ترجیحی همان جایی است که `permissions`
    #: نشسته: پیوندِ کاربر و کسب‌وکار.
    #:
    #: **NULL یعنی «هنوز انتخاب نکرده»** و فهرستِ پیش‌فرض نشان داده می‌شود؛ `[]` یعنی
    #: کاربر همه را برداشته و باکس خالی است. این دو یکی نیستند و ادغامشان یعنی
    #: کاربر هرگز نتواند باکس را خالی کند.
    #:
    #: اعتبارِ شناسه‌ها را سرور نمی‌سنجد — فهرستِ صفحه‌ها فقط در فرانت است. شناسه‌ی
    #: ناشناخته هنگام نمایش کنار گذاشته می‌شود، پس ماژولی که خاموش شود کارتِ مرده
    #: جا نمی‌گذارد.
    dashboard_cards: Mapped[list | None] = mapped_column(JSONB, nullable=True)
