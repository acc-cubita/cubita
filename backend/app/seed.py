...
CHART_OF_ACCOUNTS = [
    ...
    #: وامِ کارکنان **دارایی** است نه هزینه: پولی که داده‌ایم و قسط‌به‌قسط
    #: از حقوق برمی‌گردد. کدِ ۱۱۰۸ در قالبِ چارت «پیش‌پرداخت‌ها»ست، پس ۱۱۱۱.
    ("1111", "وام و مساعده‌ی کارکنان", "asset", False, "11"),
    #: وجوهِ در راهِ کارت‌خوان — پولی که مشتری کارت کشیده ولی شرکتِ پرداخت هنوز به
    #: بانک نریخته. کدِ ۱۱۰۸ در قالبِ چارتِ صنفی «پیش‌پرداخت‌ها»ست و ۱۱۰۹ و ۱۱۱۰
    #: هم گرفته‌اند، پس ۱۱۱۲.
    ("1112", "وجوهِ در راهِ کارت‌خوان", "asset", False, "11"),
    #: چکِ واگذارشده به بانک هنوز وصول نشده — دارایی هست، ولی نه «نزدِ ما».
    ("1113", "چک‌های واگذارشده به بانک", "asset", False, "11"),
    #: کالای در راه — بینِ «فاکتور ثبت شد» و «کالا رسید». بدونِ این، ترازنامه
    #: کالایی را دارایی نشان می‌دهد که هنوز در هیچ انباری نیست.
    ("1114", "کالای در راه", "asset", False, "11"),
    #: کالای در جریان ساخت — طرفِ بستانکارِ ورودِ محصولِ تولیدی به انبار. بی این
    #: حساب، رسیدِ نوعِ «تولید» صندوق را بابتِ کالایی که خودمان ساخته‌ایم کم می‌کرد.
    ("1115", "کالای در جریان ساخت", "asset", False, "11"),
    #: سپرده‌ی حسن انجامِ کار نزدِ کارفرما — تا تسویه‌حسابِ نهاییِ پیمان هیچ‌جا
    #: ثبت نمی‌شد (صورت‌وضعیت فقط رکورد است، سند نمی‌زند).
    ("1116", "سپرده‌ی حسن انجامِ کار نزدِ کارفرما", "asset", False, "11"),
    ...
    #: کسوراتِ فاکتور خرید خدمات — بدهی به سازمانِ مالیاتی و بیمه، **جدا از ۲۱۰۳**ِ
    #: حقوق. کسب‌وکارهای موجود این دو را با اولین کسر می‌گیرند (`get_or_create`).
    ("2112", "مالیات تکلیفی پرداختنی", "liability", False, "21"),
    ("2113", "حق بیمه پرداختنی اشخاص ثالث", "liability", False, "21"),
    #: کسورِ اختیاریِ حقوق که از کارمند نگه داشته می‌شود و به شخصِ ثالث بدهکاریم
    #: (بیمه‌ی تکمیلی، صندوق…). بی این حساب، سندِ حقوقِ هر قراردادی که ردیفِ
    #: کسور داشت دقیقاً به همان اندازه نامتوازن ثبت می‌شد.
    ("2109", "سایر کسور حقوق پرداختنی", "liability", False, "21"),
    #: کسرِ پیش‌پرداخت روی صورت‌وضعیت یعنی بخشی از این بدهی با کارکردِ واقعی
    #: جبران شده؛ تسویه‌حسابِ نهاییِ پیمان همان کاهش را یک‌جا ثبت می‌کند.
    ("2114", "پیش‌دریافتِ پیمان", "liability", False, "21"),
    #: جاری شرکا — بدهی، چون رابطه‌ی وام است نه مالکیت. تنها حسابِ چارت که
    #: عمداً دوطرفه است: شریک که وام می‌دهد بستانکار می‌شود و وقتی برداشتِ
    #: قابلِ بازپرداخت دارد بدهکار (قاعده‌ی ۵۲).
    ("2115", "جاری شرکا", "liability", False, "21"),
    #: **گروهِ ۲۲ تا امروز در چارتِ پایه نبود** — فقط «بدهی‌های جاری» داشتیم. در
    #: نتیجه هیچ حسابی نمی‌توانست «بلندمدت» باشد، و صورت جریان وجوه نقد که
    #: دنبالِ گروهِ ۲۲ می‌گشت هرگز چیزی پیدا نمی‌کرد. طبقه‌بندیِ جاری/غیرجاری
    #: بدونِ این گروه اصلاً ممکن نیست.
    ("22", "بدهی‌های بلندمدت", "liability", True, "2"),
    ("2201", "وام‌ها و تسهیلات بلندمدت", "liability", False, "22"),
    ...
    ("4198", "اضافات فروش", "income", False, "4"),
    ("4199", "تخفیفات فروش", "income", False, "4"),
    ...
    # کاهنده‌ی درآمد. جدا از «فروش» تا فروشِ ناخالص و برگشتی هر دو قابلِ گزارش
    # بمانند — وگرنه نرخِ برگشت از دفتر درنمی‌آید. ۴۱۰۴ عمداً گرفته نشد: قالب‌های
    # صنفی آن را «تخفیفات و برگشت از فروش» (ترکیبی) برده‌اند.
    ("4107", "برگشت از فروش", "income", False, "4"),
    # اختلافِ گِردکردنِ مبلغِ فاکتور. تا پیش از مهاجرتِ ۰۱۴۳ نقشش روی ۴۱۰۲
    # («فروش کالا - آنلاین») می‌نشست و رندها را داخلِ درآمدِ یک کانالِ فروشِ واقعی
    # گم می‌کرد. نام دقیقاً همانِ `inventory.sales_rounding_account` است تا چارتِ
    # seed‌شده و چارتِ تنبل‌ساخته دو اسمِ متفاوت برای یک حساب نداشته باشند.
    ("4108", "تعدیلِ گِرد کردن فروش", "income", False, "4"),
    #: درآمدِ پیمانکاری — جدا از «فروش» چون منشأش فروشِ کالا/خدمتِ متعارف نیست.
    ("4109", "درآمدِ پیمانکاری", "income", False, "4"),
    #: سودِ خروجِ دارایی ثابت — مازادِ مبلغِ فروش بر ارزشِ دفتری. جدا از «درآمد
    #: متفرقه» تا سودِ یک‌باره‌ی واگذاری با درآمدِ تکرارشونده قاطی نشود.
    ("4112", "سودِ خروجِ دارایی ثابت", "income", False, "4"),
    ...
    #: اختلافِ گِرد کردنِ خالصِ حقوق — هم‌خانواده‌ی «تعدیل رند فروش». داخلِ
    #: هزینه‌ی حقوق گم نمی‌شود، وگرنه هزینه‌ی دفتر با جمعِ فیش‌ها نمی‌خواند.
    ("5118", "تعدیل رند حقوق", "expense", False, "5"),
    #: سایرِ کسوراتِ صورت‌وضعیت (بیمه/جریمه/مالیاتِ واردشده‌ی دستی) — جدا از
    #: «هزینه خرید خدمات» چون آن برای خریدِ خدمت است، نه کسرِ دریافتی از فروش.
    ("5119", "سایرِ کسوراتِ صورت‌وضعیت", "expense", False, "5"),
    #: زیانِ خروجِ دارایی ثابت — وقتی ارزشِ دفتری از مبلغِ دریافتی بیشتر باشد
    #: (اسقاط و اهدا همیشه اینجا می‌نشینند). جدا از «هزینه‌های عمومی» چون رویدادِ
    #: یک‌باره است نه هزینه‌ی جاری؛ ۵۱۲۰–۵۱۲۳ را قالب‌های صنفی گرفته‌اند.
    ("5124", "زیانِ خروجِ دارایی ثابت", "expense", False, "5"),
]
