...
def update_check_status(
    ...
    *,
    ...
    #: **گذری که مانده‌ی طرف حساب را برمی‌گرداند باید بگوید مالِ کیست.**
    #: واخواست و استرداد هر دو همان دریافتنی/پرداختنیِ صاحبِ چک را احیا می‌کنند —
    #: ولی تا امروز رویدادشان `contact_id` نداشت، پس آن احیا به هیچ‌کس نسبت داده
    #: نمی‌شد: در نمای اقلامِ باز، چکِ برگشت‌خورده همچنان «پرداختِ مشتری» به نظر
    #: می‌رسید و می‌شد بابتش فاکتور تسویه کرد.
    #:
    #: خرج‌کردن و برگشتِ خرجِ چک عمداً این‌جا نیستند: آن‌ها پرداختنیِ **گیرنده‌ی
    #: چک** را تکان می‌دهند، نه صاحبِ اولیه‌اش. هرکس آن مسیر را صدا می‌زند خودش
    #: باید طرف حسابِ درست را بدهد.
    if contact_id is None and new_status in ("bounced", "returned"):
