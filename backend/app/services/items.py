...
def assert_batch_toggle_allowed(db: Session, item: Item, new_value: bool) -> None:
    """«ردیابیِ بار» فقط وقتی روشن می‌شود که هر واحدِ موجود به یک بار منتسب باشد.

    **چرا گاردِ `is_serial_tracked` این‌جا کپی نشد.** آن یکی می‌گوید «گردشِ
    انباری دارد → رد»، و برای سریال درست بود: سریال به دنیایی اضافه شد که هیچ
    سریالی نداشت. ولی **بچ از قبل وجود دارد** — هر خرید خودکار `P{n}-{i}`
    می‌سازد و هر رسیدِ انبار `WR{n}-{s}`. پس گاردِ تاریخی این قابلیت را دقیقاً
    برای کالاهایی که سال‌ها خرید داشته‌اند — یعنی همان‌هایی که مهم‌اند — برای
    همیشه غیرقابلِ دسترس می‌کرد.

    معیارِ درست **کمّی** است: تا وقتی موجودیِ بی‌برچسب صفر نشده، جمعِ بارها هرگز
    با موجودیِ کالا برابر نمی‌شود و هر عددی که نشان بدهیم غلط است.

    و بن‌بست نیست: پیام راهِ عبور را می‌گوید. «انتسابِ موجودی به بار» یک جفتِ
    جمعِ صفر می‌نویسد و کار تمام است.

    **خاموش‌کردن آزاد است** — برخلافِ سریال. برچسب‌های گذشته معنادار می‌مانند و
    هیچ‌وقت حذف نمی‌شوند؛ خاموشی فقط یعنی «از این به بعد سندِ خروج مجبور نیست
    بار را نام ببرد».
    """
    if new_value == item.is_batch_tracked:
        return
    if not new_value:
        return
    from app.services import batches as batches_svc

    untagged = batches_svc.untagged_on_hand(db, item.id)
    if untagged != 0:
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            f"«{item.name}» {untagged.normalize():f} واحد موجودیِ بدونِ بار دارد و تا وقتی "
            "این عدد صفر نشود، مانده‌ی بارها با موجودیِ کالا نمی‌خواند. "
            "از «انتسابِ موجودی به بار» استفاده کنید تا موجودیِ فعلی روی یک بارِ "
            "اول‌دوره بنشیند، بعد این گزینه را روشن کنید.",
        )
