...
def amounts_from_lines(db: Session, lines) -> dict[str, Decimal]:
    ...
    for line in lines:
        ...
        #: **عاملِ متغیر روی حکم نمی‌نشیند** (مهاجرتِ ۰۱۴۷). حکم شرایطِ *ماندگارِ*
        #: استخدام را می‌گوید و مبلغی که هر ماه فرق می‌کند شرطِ ماندگار نیست.
        #:
        #: و اگر می‌نشست، عددِ ماه دو بار شمرده می‌شد: یک‌بار از ستونِ حکم و
        #: یک‌بار از ورودیِ دوره.
        #:
        #: گارد — مثلِ گاردِ `is_active` بالا — فقط روی **ذخیره‌ی تازه** است:
        #: حکم‌های موجود و فیش‌های صادرشده دست نمی‌خورند.
        if factor.kind == "variable":
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST,
                f"عاملِ «{factor.name}» متغیر است و مبلغش هر دوره جدا وارد می‌شود، "
                "نه روی حکم. از «ورودی عوامل دوره» واردش کنید؛ یا اگر مبلغش ثابت "
                "است، نوعِ عامل را به «قراردادی (ثابت)» تغییر دهید.",
            )
