...
def generate_payslips_for_period(db: Session, period_id: UUID, user: User) -> list[Payslip]:
    ...
    for employee in employees:
        ...
        #: **«تاریخ پایان خدمت» تا امروز خوانده نمی‌شد.** کاربر پرش می‌کرد،
        #: قیدِ دیتابیس اعتبارش می‌سنجید، و `employees.termination_date` را هم
        #: پر می‌کرد — ولی هیچ محاسبه‌ای نگاهش نمی‌کرد. چون
        #: `get_current_contract` فقط `effective_from <= as_of` را می‌بیند،
        #: حکمِ کسی که رفته بود همچنان «حکمِ جاری» می‌ماند و هر دوره یک فیشِ
        #: کامل می‌ساخت. docstringِ خودِ `SalaryContract` صریح است: «از این‌جا
        #: به بعد کارکرد اصلاً محاسبه نمی‌شود».
        #:
        #: گارد **این‌جاست نه داخلِ `get_current_contract`** — چون سنوات و
        #: بازخریدِ مرخصی (`services/benefits.py`) همان تابع را دقیقاً برای
        #: کارمندِ *رفته* صدا می‌زنند؛ فیلترکردن آن‌جا محاسبه‌ی سنوات را می‌شکست.
        #:
        #: `valid_until` عمداً این‌جا نیست: «اعتبارِ حکم تمام شده» با «این آدم
        #: رفته» یکی نیست و تمدیدِ حکم اغلب دیر می‌رسد. قطعِ خودکارِ حقوقِ
        #: کارمندِ شاغل خرابیِ بدتری از آن چیزی بود که می‌بست.
        if contract.service_end_date is not None and contract.service_end_date < as_of:
            continue
