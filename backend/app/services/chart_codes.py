...
#: سود و زیانِ **خروجِ دارایی ثابت** (فروش/اسقاط/اهدا) — دو حسابِ جدا، نه یک حسابِ
#: خالصِ دوطرفه.
#:
#: **چرا از «درآمد متفرقه» و «هزینه‌های عمومی» جداست:** خروجِ دارایی درآمدِ عملیاتی
#: نیست و هزینه‌ی جاری هم نیست؛ رویدادی است که یک‌بار اتفاق می‌افتد و صورتِ سود و
#: زیان باید بتواند جدایش کند. ریختنش در «درآمد متفرقه» یعنی سودِ یک‌باره‌ی فروشِ
#: ساختمان با درآمدِ تکرارشونده قاطی می‌شود و حاشیه‌ی سودِ سالِ بعد ناگهان افت
#: می‌کند بی‌آنکه چیزی در کسب‌وکار عوض شده باشد.
#:
#: **چرا دو حساب و نه یکی:** همان استدلالِ `FX_GAIN`/`FX_LOSS` و
#: `SALES_RETURN` — سودِ ناخالصِ واگذاری‌ها و زیانِ ناخالصشان دو حقیقتِ جدا هستند.
#: یک حسابِ خالص، واگذاریِ پرسود و واگذاریِ زیان‌ده را در یک عدد گم می‌کند.
...
DEFAULT_CODE_BY_ROLE = {
    ...
    #: ۴۱۱۲/۵۱۲۴ — نزدیک‌ترین کدهای آزاد در هر دو (seed و `chart_templates.json`)؛
    #: ۴۱۱۰/۴۱۱۱ و ۵۱۲۰–۵۱۲۳ را قالب‌های صنفی گرفته‌اند، پس گرفتنشان اینجا باعث
    #: می‌شد اعمالِ قالب آن حساب‌ها را بی‌صدا رد کند (همان تله‌ی ۴۱۰۴/۵۱۰۷).
    ASSET_DISPOSAL_GAIN: "4112",
    ASSET_DISPOSAL_LOSS: "5124",
}
