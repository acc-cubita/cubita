...
from app.services.voiding import _guard_no_active_returns
...
def cancel_unposted_sales_invoice(
    ...
    #: **همان گاردِ مسیرِ سنددار.** فاکتورِ سندنشده هم می‌تواند برگشتِ فعال داشته
    #: باشد، و لغوش بدونِ این بررسی یک سندِ برگشتِ بی‌پدر جا می‌گذارد که همچنان
