...
سندِ **تسعیرِ ارز** هم با همین استدلال مسدود نمی‌شود: ردیف‌هایش ابعادشان را از
*مانده‌ای* که تسعیر می‌کنند کپی می‌کنند، نه از انتخابِ کاربر. بدونِ این، مانده‌ای که
پیش از سخت‌گیرترشدنِ تنظیم بی‌تفصیلی ثبت شده برای همیشه غیرقابلِ تسعیر می‌ماند —
چون سند ویرایشِ ردیف ندارد و راهِ دیگری هم نیست. سوراخی باز نمی‌شود: همان ردیف در
`find_missing_tafsili` دیده می‌شود — گزارش، نه گارد.
"""
...
DIMENSION_COPYING_SOURCES = ("fx_revaluation",)
...
def _blocks(mode: str, source_type: str) -> bool:
    ...
    if source_type.startswith("void_") or source_type in DIMENSION_COPYING_SOURCES:
        return False  # ابعادش کپیِ داده‌ی موجود است، نه انتخابِ کاربر
    if mode == "floating":
        return False
    if mode == "strict":
        return True
