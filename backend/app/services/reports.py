        ...
        rows.append(
            {
                ...
                #:
                #: `average_cost` از مهاجرتِ ۰۱۲۵ چهار رقم اعشار دارد (بهای
...
def get_aging(db: Session, kind: str, as_of: date | None) -> dict:
    ...
    **این FIFO یک تقریبِ نمایشی است، نه تخصیصِ واقعی.** هیچ‌جا ذخیره نمی‌شود و
    کاربر نمی‌تواند تغییرش دهد؛ فقط کمک می‌کند مانده‌ی سطح‌شخص در سطل‌های سنی
    پخش شود. جوابِ «کدام دریافت بابتِ کدام فاکتور بود» جای دیگری است:
    «تسویه حساب طرف مقابل» و گزارشِ اقلامِ باز (`services/open_items.py`)، که
    تخصیصِ *ثبت‌شده* را می‌خوانند نه حدسِ لحظه‌ی گزارش. جمعِ هر دو یکی است؛ توزیعشان
    می‌تواند فرق کند و آن یکی حقیقت است.
    """
