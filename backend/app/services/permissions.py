...
PERMISSION_MODULES: list[dict] = [
    ...
    {
        # برچسب عمداً «ردِ تغییرات» است نه «گزارش حسابرسی»: ماژولِ تازه‌ی `assurance`
        # هم «حسابرسی» نام دارد و دو ردیفِ هم‌نام در ماتریسِ مجوزها، مالک را به
        # اشتباه می‌انداخت. این کلید همان دفترِ «چه کسی چه چیزی را عوض کرد» است.
        "key": "audit",
        "label": "ردِ تغییرات",
    ...
    },
    {
        "key": "assurance",
        "label": "حسابرسی",
        "hint": "درخواستِ حسابرسی و دیدنِ کارنامه‌ی سلامتِ دفتر",
        # `refresh` عمداً کنشِ جدایی است و نه `create`: `create` در `WRITE_ACTIONS`
        # است و گیتِ اشتراک، اجرای دوباره‌ی بررسی‌ها را روی حسابِ منقضی ۴۰۲ می‌کرد —
        # دقیقاً همان دفتری که بیش از همه به بررسی نیاز دارد. اجرای بررسی هم چیزی
        # در دفتر نمی‌نویسد، فقط عکس می‌گیرد.
        "actions": ["view", "create", "refresh"],
    },
    {
        "key": "users",
