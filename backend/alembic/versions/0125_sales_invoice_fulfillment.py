...
def upgrade() -> None:
    ...
    for column in (
        ...
        #: `tax_rate_snapshot` و `tax_amount_snapshot` این‌جا **نیستند**: مهاجرتِ
        #: ۰۱۱۷ از قبل ساخته‌شان (فصلِ کالا و خدمت، مالیاتِ ردیف‌به‌ردیف). ساختنِ
        #: دوباره‌شان روی دیتابیسِ مستقر با `DuplicateColumn` می‌شکست — همان
        #: تصادمی که `git` نشان نمی‌دهد چون دو فایلِ متفاوت‌اند.
    ):
...
def downgrade() -> None:
    ...
    #: دو ستونِ مالیاتِ ردیف مالِ مهاجرتِ ۰۱۱۷ است و این‌جا انداخته نمی‌شود —
    #: وگرنه downgradeِ این مهاجرت چیزی را می‌برد که خودش نساخته بود.
    for name in (
        "unit_snapshot", "item_name_snapshot", "item_code_snapshot", "duty_amount", "addition",
    ):
