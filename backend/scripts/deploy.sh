...
# و **نباید** آدرسِ اپِ ستاد را داشته باشد. گاردِ بالا فقط می‌گوید آدرسِ درست
# هست؛ نمی‌گوید آدرسِ غلط نیست. از وقتی دو باندلِ هم‌شکل داریم (اپِ مشتری و
# اپِ ستاد)، بسته‌ی اشتباه می‌توانست از گاردِ بالا رد شود و کلِ اپ را عوض کند.
if grep -rqF "https://admin.cubita.ir" "$APP_DIR/web"/assets/*.js 2>/dev/null; then
    echo "این باندلِ اپِ ستاد است، نه اپِ مشتری — استقرار متوقف شد." >&2
    false
fi
